  ...
  useEffect(() => {
    ...
    const draw = (t: number) => {
      ...
      if (endingPage === 0) {
        ...
        drawPanda(ctx, 650, 270, seconds, "peace", -1, 3.2, true); ctx.restore();
      }
      const walking = endingPage === 3;
      const x = walking ? 440 + Math.min(180, seconds * 22) : 480;
      drawPanda(ctx, x, 290, seconds, walking ? "walk" : mood === "intro" ? "idle" : "peace", endingPage === 2 ? -1 : 1, mood === "intro" ? 2.4 : 2.9);
    ...
    art.forest.onload = () => draw(performance.now()); raf = requestAnimationFrame(frame); document.addEventListener("visibilitychange", resume);
    return () => { cancelAnimationFrame(raf); art.forest.onload = null; document.removeEventListener("visibilitychange", resume); };
