...
export function createMemoryStore() {
  ...
  return {
    ...
    async createGroup({ invite_token } = {}) {
      const group = { id: (seq.group += 1), name: "My board", invite_token: invite_token ?? null };
      groups.push(group);
      return clone(group);
    },
    ...
    async getUserByLastIp(ip) {
      const matches = users.filter((u) => u.last_ip === ip && u.last_seen_at);
      if (!matches.length) return null;
      matches.sort((a, b) => (a.last_seen_at < b.last_seen_at ? 1 : -1));
