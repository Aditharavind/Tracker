...
--
-- Already ran an older copy of this file (no invite_token / last_ip
-- columns)? Run supabase/migration-04.sql instead, which adds them in place.
-- =====================================================================
...
create table if not exists public.groups (
  ...
  -- Unguessable link anyone can use to join this group as a new, editable
  -- member -- distinct from a user's own share_token, which stays read-only.
  invite_token text,
...
create table if not exists public.users (
  ...
  -- Convenience only, never an auth mechanism: lets a browser with no saved
  -- local user get its tile pre-selected on a later visit from the same IP
  -- instead of landing on the onboarding screen. PIN is still required for
  -- every mutation regardless of IP -- see GET /api/session/suggest.
  last_ip       text,
...
-- the session-suggestion lookup: most recent user seen from a given IP
